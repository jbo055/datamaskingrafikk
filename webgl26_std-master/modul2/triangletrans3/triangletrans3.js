...
/**
 * Et WebGL-program som tegner en mange trekanter.
...
export function main() {
	...
	// Sett tilfeldig posisjon og størrelser på trekantene:
...
function draw(renderInfo) {
	...
	for (let i=0; i < renderInfo.triangles.length; i++) {
		...
		drawTriangle(renderInfo, modelMatrix, renderInfo.triangles[i].color);
	}
}
...
function drawTriangle(renderInfo, modelMatrix, color) {
	...
	let triangleColor = [color.r, color.g, color.b, color.a];
	connectColorUniform(renderInfo.gl, renderInfo.baseShaderInfo, triangleColor);
	...
	// Kople til farge uniform:
	color = [0,0,0,1];
	connectColorUniform(renderInfo.gl, renderInfo.baseShaderInfo, color);
