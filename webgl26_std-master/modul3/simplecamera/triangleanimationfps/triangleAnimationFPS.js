...
/**
 * Et WebGL-program som tegner en animert trekant.
...
function draw(currentTime, renderInfo, camera) {
	...
	connectPositionAttribute(gl, renderInfo.baseShader, renderInfo.trianlgeBuffers.position);
	connectColorAttribute(gl, renderInfo.baseShader, renderInfo.trianlgeBuffers.color);
    // MODEL:
    renderInfo.modelMatrix.setIdentity();
    renderInfo.modelMatrix.rotate(renderInfo.animation.angle, 0, 0, 1);
    // MODELVIEW:
	...
	// Tegn!
	gl.drawArrays(gl.TRIANGLES, 0, renderInfo.trianlgeBuffers.vertexCount);
}