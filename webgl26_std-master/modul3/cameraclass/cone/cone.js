...
function drawCone(renderInfo, camera) {
	...
	gl.useProgram(renderInfo.baseShader.program);
	// Kople posisjon og farge-attributtene til tilhørende buffer:
	connectPositionAttribute(gl, renderInfo.baseShader, renderInfo.coneBuffers.position);
	connectColorAttribute(gl, renderInfo.baseShader, renderInfo.coneBuffers.color);
    // MODEL:
    renderInfo.modelMatrix.setIdentity();
    // MODELVIEW:
    renderInfo.modelviewMatrix.set(camera.viewMatrix);
	...
	gl.drawArrays(gl.TRIANGLE_FAN, 0, renderInfo.coneBuffers.vertexCount);
}
