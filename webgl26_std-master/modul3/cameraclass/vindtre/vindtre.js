 ...
 * ligger i lokal (0, -halvLengde, 0), og forelderes endepunkt (toppen) i lokal (0, +halvLengde, 0).
 ...
 *      T(0, egen halvLengde)         - Flytt delen opp slik at bunnen av delen ligger i origo (y=0), dvs. halvparten av delens høyde.
 *
 * TORS: Her brukes T og O, der O=R*T
 *
 * Merk at det som faktisk skjer med verteksene er motsatt rekkefølge:
 *   => først flyttes delen opp tilsvarende jalvparten av delens høyde, slik at bunnen av delen ligger i origo,
 *      dette blir leddet som delen roteres om.
 *   => så roteres den om leddet,
 *   => og til slutt flyttes den ut
 *
...
const STAMME = {halvLengde: 4.0, halvBredde: 0.50, farge: [0.45, 0.30, 0.15, 1.0]};
const GREN   = {halvLengde: 2.5, halvBredde: 0.35, farge: [0.50, 0.15, 0.18, 1.0]};
const KVIST  = {halvLengde: 1.5, halvBredde: 0.20, farge: [0.20, 0.60, 0.25, 1.0]};

// Grenene spriker +/- 35 grader ut fra stammen, kvistene +/- 30 grader ut fra grena:
const GRENVINKEL = 35;
const KVISTVINKEL = 30;
...
export function main() {
	...
	const renderInfo = {
		...
		animation: {
			...
			stammeVinkel: 0,    // Svinger  +/-  2 grader.
			grenVinkel: 0,      // Svinger  +/-  6 grader.
			kvistVinkel: 0      // Svinger  +/- 12 grader.
		}
	};
...
function animerVind(elapsed, renderInfo) {
	...
	anim.stammeVinkel = v *  2 * Math.sin(t * 1.1);
	anim.grenVinkel   = v *  6 * Math.sin(t * 1.7 + 0.6);
	anim.kvistVinkel  = v * 12 * Math.sin(t * 2.6 + 1.2);
}
...
/**
 * Tegner hele treet: en stamme, to grener og fire kvister.
...
function drawTree(renderInfo, camera) {
	...
	createAndPushPartMatrix(renderInfo, 0, renderInfo.animation.stammeVinkel, STAMME);
	drawPart(renderInfo, camera, STAMME.halvBredde, STAMME.halvLengde, STAMME.farge);
	for (const side of [1, -1]) {          // FORGREINING 1: to grener ut fra stammens topp/endepunkt.
		createAndPushPartMatrix(renderInfo, STAMME.halvLengde, side * GRENVINKEL + renderInfo.animation.grenVinkel, GREN);
		drawPart(renderInfo, camera, GREN.halvBredde, GREN.halvLengde, GREN.farge);

		for (const ut of [1, -1]) {        // FORGREINING 2: to kvister ut fra grenas topp/endepunkt.
			createAndPushPartMatrix(renderInfo, GREN.halvLengde, ut * KVISTVINKEL + renderInfo.animation.kvistVinkel, KVIST);
			drawPart(renderInfo, camera, KVIST.halvBredde, KVIST.halvLengde, KVIST.farge);
			renderInfo.stack.popMatrix();                  // POP kvist
		}
		renderInfo.stack.popMatrix();                       // POP gren
	}
	renderInfo.stack.popMatrix();                          // POP stamme
 ...
 * Merk at vi bruker en KOPI av matrisen som ligger øverst på stacken.
 * Dette sikrer at skaleringa ikke påvirker matrisa som ligger på toppen av stacken,
 * og som ev. brukes videre av andre "barn" av aktuell del.
 */
